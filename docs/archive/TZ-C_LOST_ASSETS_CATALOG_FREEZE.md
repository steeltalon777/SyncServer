...
- [x] 6. UI automation tests complete (not applicable — no UI changed in this TZ)
...
| Unit tests (repo) | `python -m pytest tests/test_catalog_freeze.py -k "test_repo"` | pass (4/4) | 4 repo-level tests cover all frozen/unfrozen conditions |
| Component/route tests | `python -m pytest tests/test_catalog_freeze.py -k "test_update or test_delete"` | pass (3/3) | 3 service-level tests check 409 on update/delete and no-error on unfrozen |
...
| Stand smoke | `SYNC_TEST_MODE=stand SYNC_TEST_ALLOW_STAND=1 SYNC_TEST_BASE_URL=http://localhost:8000 SYNC_TEST_ROOT_TOKEN=... python -m pytest tests/stand/smoke/test_stand_smoke.py -m stand` | pass (4/4) | Health, readiness, auth, run_id — stand is up, root token authenticates against live SyncServer |
| UI automation | N/A | N/A | No UI changed in this TZ (per TZ section 7) |
| Regression pack | `python -m pytest tests/test_catalog_admin_soft_delete.py tests/test_lost_assets_api.py tests/test_operations_acceptance_and_issue_api.py` | pass (10 pass, 6 xfail) | Existing catalog soft delete, lost assets API, operations acceptance all pass unchanged |
...
- Reviewed: implementation, all 8 freeze tests, regression pack, live stand smoke, and existing service code.
- Freeze invariant: `CatalogAdminService.update_item` / `delete_item` return stable 409 `"item is frozen by active lost asset balance"` when a positive `lost_asset_balances.qty` exists for the item's `inventory_subjects` row.
- `test_full_flow_lost_resolve_unfreeze` proves: create operation → acceptance with lost qty → catalog update blocked (409) → write-off resolves lost balance → catalog update succeeds (200).
- Regression pack unchanged: 10 pass, 6 xfail (expected).
- Stand smoke confirms the live SyncServer at `localhost:8000` is healthy and the root token from bootstrap authenticates.
- Verdict: **Accepted**.
