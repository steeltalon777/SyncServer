...
- [x] 6. UI automation tests complete (not applicable — no UI touched)
...
|---|---|---|---|
| Static checks | `python -m compileall scripts app tests` | pass | Only `__pycache__` lock conflicts; source compiles clean |
| Unit/script tests | `python -m pytest tests/test_bootstrap_root.py -v` | pass (17/17) | 5 first-run + 5 idempotency + 7 rotation tests; covers all 6 required scenarios from §5 L3 |
| DB integration | same suite, real test DB via fixtures | pass | Tests use real `db_session` and `client` (ASGI transport) against PostgreSQL test DB |
| Stand smoke | `SYNC_TEST_MODE=stand SYNC_TEST_ALLOW_STAND=1 SYNC_TEST_BASE_URL=http://localhost:8000 SYNC_TEST_ROOT_TOKEN=... python -m pytest tests/stand/smoke/test_stand_smoke.py -m stand` | pass (4/4) | Health, readiness, auth, run_id — root token from bootstrap authenticates against real stand |
| Regression auth/admin | `python -m pytest tests/test_auth_smoke.py tests/test_auth_unified.py tests/test_admin_root_permissions.py tests/test_admin_device_ensure.py` | pass (40/40) | Existing auth/admin/device tests unchanged |
| Documentation | `SyncServer/docs/TEST_STAND_GUIDE.md` §0 "Инициализация БД и bootstrap" | done | Documents first-time bootstrap, idempotent re-run, `--run-migrations`, explicit rotation, env var names `SYNC_ROOT_USER_TOKEN` / `SYNC_DEVICE_TOKEN` |

## 10. Reviewer Notes (QA)

- Reviewed: implementation, tests, stand smoke, regression pack, and documentation.
- `bootstrap_root.py` and `rotate_tokens.py` are correctly idempotent: re-run does not rotate tokens; explicit `--root` / `--django-device` flags rotate only the target token.
- Output uses Django env var names and clearly prints the previous and new tokens.
- Stand is up (`/api/v1/health` → ok). Root token from a bootstrap-equivalent state authenticates against the live SyncServer.
- Verdict: **Accepted**.
